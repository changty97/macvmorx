...
func NewOrchestrator(ns *store.NodeStore, cfg *config.Config) (*Orchestrator, error) {
	// Setup mTLS client for communicating with agents
	...
	tlsConfig := &tls.Config{
		RootCAs:      caCertPool,                    // Trust server certificates signed by our CA
...
func (o *Orchestrator) InstructAgentToProvisionVM(nodeID string, cmd models.VMProvisionCommand) error {
	// Use HTTPS for communication with agents
	agentURL := fmt.Sprintf("https://%s:8081/provision-vm", nodeID) // Assuming nodeID can be resolved to IP
	...
	resp, err := o.httpClient.Do(req) // Use the pre-configured mTLS client
	if err != nil {
