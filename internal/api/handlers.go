...
import (
	...
	"net/http"
	"strings"
	"time" // Added for InstructAgentToProvisionVM
...
func (h *Handlers) HandleGitHubWebhook(w http.ResponseWriter, r *http.Request) {
	...
	// Corrected: Pass the *http.Request and handle two return values
	...
	case "workflow_job":
		...
		if event.Action != nil && *event.Action == "queued" {
			...
			isSelfHosted := false
			// Corrected: Access Labels directly
			...
			// Corrected: Access GitHubRunnerRegistrationToken directly from h.cfg
			...
			log.Printf("Self-hosted workflow job '%s' (%d) queued for repository %s. Labels: %v",
				event.WorkflowJob.GetName(), event.Repo.GetFullName(), event.WorkflowJob.Labels) // Corrected: Access Labels directly
			...
			go func() {
				...
				nodeID, err := h.Orchestrator.ScheduleVM(
					...
					event.WorkflowJob.Labels, // Corrected: Access Labels directly
					imageName,
					h.cfg.GitHubRunnerRegistrationToken, // Corrected: Access GitHubRunnerRegistrationToken directly
				)
				if err != nil {
